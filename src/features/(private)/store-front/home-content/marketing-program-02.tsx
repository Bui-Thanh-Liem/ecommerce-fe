...
interface MarketingProgram02Props {
  idConfig: string
  mktProgram02: { campaigns: CampaignOption[] } | undefined
}

export function MarketingProgram02({
  idConfig,
  mktProgram02,
}: MarketingProgram02Props) {
  // Lấy danh sách các campaigns có sẵn từ API công khai/options
  ...
  useEffect(() => {
    if (mktProgram02) {
      if (mktProgram02.campaigns) {
        // eslint-disable-next-line react-hooks/set-state-in-effect
        setSelectedCampaigns(mktProgram02.campaigns)
      }
    }
  }, [mktProgram02])

  // Xử lý khi Combobox thay đổi danh sách ID được chọn
  const handleCampaignsChange = (selectedIds: string[]) => {
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        ...
        <CustomCombobox
          ...
          searchPlaceholder="Search campaigns names..."
              ...
              >
                ...
                {campaign.mainImage?.url ? (
                  <div className="bg-muted relative h-16 w-32 overflow-hidden rounded-lg">
                    <Image
                      ...
                      alt={campaign.name || "Campaigns preview"}
                    />
                  </div>
                ) : (
                ...
                <div className="flex min-w-0 flex-1 flex-col justify-center">
                  <span className="truncate text-sm font-semibold">
                    {campaign.name || "Chưa đặt tên chương trình"}
                  </span>
