...
//
export function InternalChatbotPage() {
  const { data } = useFindAllRagDocuments("internal")
  ...
  async function onSubmit(values: IngestDocumentDto) {
    ...
    try {
      ...
      await mutateAsync({ file: fileToUpload, type: "internal" })
  ...
  async function handleDeleteDocument(id: string) {
    try {
      await deleteDocumentMutation({ id, type: "internal" })
      ...
      <div className="flex items-center justify-between border-b border-slate-200 pb-5">
        <div>
          <h1 className="flex items-center gap-x-3 text-3xl font-bold tracking-tight text-slate-900">
            ...
            ></span>
            Database Management -
            <span className="inline-block rounded-4xl bg-red-500 p-1 px-4 text-white">
              Internal
            </span>
            Chatbot
          </h1>
          <p className="mt-1 text-sm text-slate-500">
            Manage data for the internal chatbot, including uploading documents,
            tracking embedding status, and monitoring storage usage statistics.
          </p>
        ...
        <div className="rounded-3xl border border-slate-100 bg-white p-6 shadow-sm">
          ...
          <form onSubmit={form.handleSubmit(onSubmit)}>
            ...
            <Button
              ...
              className="mt-7 h-11 w-full rounded-xl font-medium text-white shadow-md transition-opacity hover:opacity-95"
            >
              {isUploading ? (
                <>
