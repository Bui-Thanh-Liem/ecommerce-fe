...
import { useCreateStaff, useFindAllStaffs } from "@/hooks/use-staff"
import { useFindAllStores } from "@/hooks/use-store"
import { CreateStaffSchema } from "@/shared/dtos/req/create-staff.dto"
...
export function StaffAddForm() {
  ...
  //
  async function onSubmit(data: z.infer<typeof CreateStaffSchema>) {
    try {
      // API không cần confirmPassword nên xóa trước khi gửi
      delete data.confirmPassword

      //
      const res = await createStaffApi.mutateAsync(data)
                    ...
                    <SelectContent align="end">
                      <SelectGroup>
                        {[
                          ...
                          {
                            id: form.watch("store") || "store-undefined",
                            name:
                              stores.find((s) => s.id === form.watch("store"))
                                ?.name || "Belong to Store",
                          },
                        ].map((workLocation) => (
                          <SelectItem
                            key={workLocation.id}
